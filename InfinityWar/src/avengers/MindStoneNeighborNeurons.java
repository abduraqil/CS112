...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        int numberofNuerons = StdIn.readInt();

        AdjacencyList2 graph = new AdjacencyList2(numberofNuerons);
        for (int i = 0; i < numberofNuerons; i++) {
            graph.setNode(StdIn.readString());
        }

        int numConnect = StdIn.readInt();
        for (int i = 0; i < numConnect; i++) {
            graph.setEdge(StdIn.readString(), StdIn.readString(), 1);
        }

        StdOut.setFile(MindStoneNeighborNeuronsOutputFile);
        String[] edges = graph.getNodeEdges(numberofNuerons-1);
        for (int i = 0; i < edges.length; i++) {
            String string = edges[i];
